...
pol = FeedWhenCrying()
r1 = BS_DESPOT.branching_sim(pomdp, pol, b, 10, (m,x)->0.0)
r2 = BS_DESPOT.branching_sim(pomdp, pol, b, 10, (m,x)->0.0)
@test r1 == r2
tval = 7.0
r3 = BS_DESPOT.branching_sim(pomdp, pol, b, 10, (m,x)->tval)
@test r3 == r2 + tval*length(b.scenarios)

scenarios = [1=>rand(rng, b_0)]
b = ScenarioBelief(scenarios, rs, 0, false)
pol = FeedWhenCrying()
r1 = BS_DESPOT.rollout(pomdp, pol, b, 10, (m,x)->0.0)
r2 = BS_DESPOT.rollout(pomdp, pol, b, 10, (m,x)->0.0)
@test r1 == r2
tval = 7.0
r3 = BS_DESPOT.rollout(pomdp, pol, b, 10, (m,x)->tval)
@test r3 == r2 + tval
...
solver = BS_DESPOTSolver(bounds=bds)
...
solver = BS_DESPOTSolver(epsilon_0=0.1,
                      bounds=bds,
...
D = @inferred BS_DESPOT.build_despot(p, b0)
@inferred BS_DESPOT.explore!(D, 1, p, CPUtime_us())
@inferred BS_DESPOT.expand!(D, length(D.children), p)
@inferred BS_DESPOT.prune!(D, 1, p)
@inferred BS_DESPOT.find_blocker(D, length(D.children), p)
@inferred BS_DESPOT.make_default!(D, length(D.children))
@inferred BS_DESPOT.backup!(D, 1, p)
@inferred BS_DESPOT.excess_uncertainty(D, 1, p)
...
# from README:
using POMDPs, POMDPModels, POMDPSimulators, BS_DESPOT

pomdp = TigerPOMDP()

solver = BS_DESPOTSolver(bounds=(-20.0, 0.0))
planner = solve(solver, pomdp)
