...
function explore!(D::PL_DESPOT, b::Int, p::PL_DESPOTPlanner)
    if D.Delta[b] <= p.sol.D &&
        ...
        next_beliefs = next_best(D, b, p)
        for next_belief in next_beliefs
            explore!(D, next_belief, p)
        end
    end
    if D.Delta[b] > p.sol.D
        make_default!(D, b)
...
function prune!(D::PL_DESPOT, b::Int, p::PL_DESPOTPlanner)
    x = b
    blocked = false
    while x != 1
...
function find_blocker(D::PL_DESPOT, b::Int, p::PL_DESPOTPlanner)
    len = 1
...
function make_default!(D::PL_DESPOT, b::Int)
    l_0 = D.l_0[b]
    D.mu[b] = l_0
    D.l[b] = l_0
end

function backup!(D::PL_DESPOT, b::Int, p::PL_DESPOTPlanner)
...
function next_best(D::PL_DESPOT, b::Int, p::PL_DESPOTPlanner)
    max_mu = -Inf
...
function excess_uncertainty(D::PL_DESPOT, b::Int, p::PL_DESPOTPlanner)
