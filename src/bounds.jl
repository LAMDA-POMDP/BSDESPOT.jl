...
function init_bounds(bounds::IndependentBounds, pomdp::POMDP, sol::BS_DESPOTSolver)
...
function init_bound(ub::FullyObservableValueUB{S}, pomdp::POMDP, sol::BS_DESPOTSolver) where S <: Solver
...
function init_bound(lb::DefaultPolicyLB{S}, pomdp::POMDP, sol::BS_DESPOTSolver) where S <: Solver
    policy = solve(lb.policy, pomdp)
    return init_bound(DefaultPolicyLB(policy, lb.max_depth, lb.final_value), pomdp, sol)
end

function init_bound(lb::DefaultPolicyLB{P}, pomdp::POMDP, sol::BS_DESPOTSolver) where P <: Policy
